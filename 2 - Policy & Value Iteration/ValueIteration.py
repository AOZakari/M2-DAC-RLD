    ...
    def __init__(self, action_space, states, mdp, alpha = 0.99, eps= 10e-4):
        ...
        while (np.linalg.norm(self.V-V_old)>eps):
            ...
            for s in self.non_final_states:
                ...
                self.V[s] = np.max([np.sum([t[0]*(t[2] + alpha*V_old[t[1]]) for t in self.mdp[s][a]]) for a in range(self.action_space.n)])
        for s in self.non_final_states:
...
if __name__ == '__main__':
    ...
    agent = ValueAgent(env.action_space, states, mdp, 0.999999)
